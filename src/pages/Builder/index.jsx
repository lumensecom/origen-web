...
const BuilderView = ({ onAddToCart, editingOrder = null, onSaveEdit, onCancelEdit }) => {
  ...
  const toggleDrink = (drink) => {
    setAddedDrinks(prev =>
      prev.find(d => d.id === drink.id) ? prev.filter(d => d.id !== drink.id) : [...prev, drink]
    );
  };
  ...
  const handleFinish = () => {
    ...
    addedDrinks.forEach(drink => onAddToCart(drink));
    resetBuilder();
  };
                ...
                {curr?.id === 'proteina' ? (
                  <div className="space-y-6">
                    ...
                    <p className="font-ui text-xs text-[var(--verde-palido)] italic">
                      {proteinMode === 'sencilla' ? 'Selecciona tu proteína favorita para una porción clásica.' : '¡Arma tu Origen Máximo! Elige hasta 2 proteínas diferentes.'}
                    </p>
          ...
          {step === 7 && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex-1 bg-[var(--verde-bosque)] p-8 rounded-[24px] border border-[var(--verde-main)]/20">
              ...
              <div className="space-y-3 font-ui text-[var(--verde-palido)] mb-6 text-sm">
                ...
                {addedDrinks.length > 0 && <p>• <strong className="text-white">Bebidas:</strong> {addedDrinks.map(d => d.nombre).join(', ')}</p>}
              </div>
